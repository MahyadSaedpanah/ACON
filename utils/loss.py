...
class WeightedNTXentLoss(torch.nn.Module):
    """
    Weighted cross-modal InfoNCE / NT-Xent for paired embeddings.
    z1, z2: [B, D], positives are (z1[i], z2[i]).
    w: [B] weights in [0,1], higher => stronger contribution.
    """
    def __init__(self, temperature=0.2, eps=1e-8):
        super().__init__()
        self.tau = temperature
        self.eps = eps

    def forward(self, z1, z2, w):
        z1 = F.normalize(z1, dim=1)
        z2 = F.normalize(z2, dim=1)
        ...
        # per-sample CE
        loss_12 = F.cross_entropy(logits, labels, reduction='none')      # [B]
        loss_21 = F.cross_entropy(logits.t(), labels, reduction='none')  # [B]
        loss = 0.5 * (loss_12 + loss_21)                                 # [B]

        w = w.detach()
        return (w * loss).sum() / (w.sum() + self.eps)
